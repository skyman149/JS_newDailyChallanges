...
codes, the most notorious of those is the international distress signal SOS (that was
     first issued by Titanic), that is coded as ···−−−···. These special codes are 
...
Elixir: @morse_codes variable (from use MorseCode.Constants). Ignore the unused 
variable warning for morse_codes because it's no longer used and kept only for old
 solutions.
...
errors and exceptions. In C#, tests will fail if the solution code throws an exception,
...
decodeMorse = function(morseCode){
    let result = '';
    let wordsArr = morseCode.split('   '); // split by 3 spaces (arr of words)
    for (let el in wordsArr) {
      let codeArr = wordsArr[el].split(' '); // split by 1 space (arr of every code)
      for (let ch in codeArr) {
        if (MORSE_CODE[codeArr[ch]] !== undefined) { // present in obj
          result += MORSE_CODE[codeArr[ch]];
        }
      }
      result += ' ';
    }  
